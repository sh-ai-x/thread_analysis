...
- `src/thread_analysis/sns_client.py` — module containing:
  - `class ThreadsAuth` — OAuth 2.0 helper. Methods:
    ...
    - `refresh_if_needed(token: ThreadsToken) -> ThreadsToken` — refreshes a token if it's within 7 days of expiry. **Self-review correction**: PKCE's `code_verifier` is single-use, scoped to one authorization-code exchange — it CANNOT be replayed for a refresh. Refresh uses the standard OAuth `grant_type=refresh_token` flow: `token.refresh_token` (see the `ThreadsToken` field below) is sent in the POST body; no `code_verifier` involved. Goes through `HttpClient.post(...)` against the refresh endpoint. Returns a new `ThreadsToken` carrying the rotated `refresh_token` if the provider rotates it (Meta's Threads API does rotate on refresh — the returned token's `refresh_token` MUST replace the stored one).
  ...
  - `class ThreadsToken` — frozen dataclass: `access_token: str`, `expires_at: datetime`, `user_id: str`, `refresh_token: str | None = None`. **`refresh_token` added (self-review)**: `refresh_if_needed` cannot refresh anything without it — the standard OAuth refresh grant needs the `refresh_token`, not the one-time PKCE `code_verifier`. `None` is valid for tokens obtained via a flow that doesn't issue one (defensive default; Meta's Threads API does issue one on the initial exchange). **Lives in `sns_types.py`** (re-export here: `from thread_analysis.sns_types import ThreadsToken`).
...
- `tests/test_sns_auth.py` — unit tests for `ThreadsAuth` with a `FakeHttpClient`:
  ...
  - `test_refresh_if_needed_within_7_days`, `test_refresh_if_needed_outside_7_days` — both go through `post()`; the fixture `ThreadsToken` carries a non-`None` `refresh_token`, and the fake `post()` asserts the outgoing body contains `grant_type=refresh_token` + that `refresh_token` value (NOT a `code_verifier`).
  - `test_refresh_rotates_refresh_token` — the fake `post()` response includes a NEW `refresh_token`; assert the returned `ThreadsToken.refresh_token` equals the new one, not the original.
